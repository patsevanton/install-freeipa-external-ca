...
### Важное примечание о сертификате FreeIPA

Согласно официальной документации Red Hat Identity Management (которая включает FreeIPA), **сертификат, используемый для интеграции внешнего CA с FreeIPA, ДОЛЖЕН быть валидным CA-сертификатом**. Это означает, что он должен содержать расширение `basicConstraints` со значением `CA:TRUE` и/или расширение `keyUsage`, разрешающее подписывать сертификаты (например, `keyCertSign`).

Сертификат, подписанный корневым CA, устанавливается как **CA-сертификат для подсистемы Dogtag в FreeIPA**, а не как обычный серверный сертификат. Именно эта подсистема в дальнейшем будет выпускать сертификаты для хостов и сервисов внутри вашего домена. Использование для этой цели сертификата без флага CA приведёт к сбою установки.
